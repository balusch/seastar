...
ss::future<int> slow_do_sth(std::unique_ptr<T> obj) {
    return ss::sleep(1s).then([obj = std::move(obj)] () mutable { return do_sth(std::move(obj)); });
}

ss::future<int> do_sth2() {
...
ss::future<> slow_op(const std::string &str) {
    return ss::sleep(100ms).then([&str] {
        std::cout << "continuation 1: " << str << std::endl;
    }).then([&str] () {
        std::cout << "continuation 2: " << str << std::endl;
    });
}
...
static ss::future<> test_fiber() {
    ...
    return ss::when_all(ss::sleep(1s), std::move(fut), ss::make_ready_future<double>(3.14)).discard_result();
    */

    /*
    ss::future<int> fut = ss::sleep(2s).then([] { return 3; });
    return ss::when_all(ss::sleep(1s), std::move(fut), ss::make_ready_future<double>(3.14))
        .then([] (auto tup) {
            std::cout << std::get<0>(tup).available() << std::endl;
            std::cout << std::get<1>(tup).get0() << std::endl;
    ...
    */
    ...
    return ss::when_all(std::move(slow_success), std::move(slow_exception), ss::make_ready_future<double>(3.14))
        .then([] (auto tup) {
            std::cout << std::get<0>(tup).available() << std::endl;
    ...
    /*
    ss::future<int> fut = ss::sleep(2s).then([] { return 3; });
    return ss::when_all_succeed(ss::sleep(1s), std::move(fut), ss::make_ready_future<double>(3.14))
        .then([] (auto tup) {
    ...
    */
    ...
    /*
    ss::future<int> fut = ss::sleep(2s).then([] { return 3; });
    return ss::when_all_succeed(ss::sleep(1s), std::move(fut), ss::make_exception_future<char>('h'), ss::make_ready_future<double>(1.3))
        .then([] (auto tup) {
            std::cout << std::get<0>(tup) << std::endl;
            std::cout << std::get<1>(tup) << std::endl;
...
static ss::future<> slow2(int i, ss::gate &g) {
    ...
    return ss::do_for_each(boost::counting_iterator<int>(0),boost::counting_iterator<int>(10), [&g] (int i) {
        g.check();
        return ss::sleep(1s);
    }).finally([i, print_now] {
...
static ss::future<> test_gate() {
    return ss::do_with(ss::gate(), [] (ss::gate &g) {
        return ss::do_for_each(boost::counting_iterator<int>(1),
                               ...
                               [&g] (int i) {
            (void) ss::with_gate(g, [i, &g] { return slow2(i, g); });
            return ss::sleep(1s);
        }).then([&g] {
