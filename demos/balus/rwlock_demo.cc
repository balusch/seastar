...
static ss::future<> f() {
    ...
    futs.emplace_back(lock.read_lock().then([]() {
    ...
    futs.emplace_back(lock.write_lock().then([]() {
    ...
    futs.emplace_back(ss::sleep(2s).then([&lock]() {
    ...
    return ss::when_all(futs.begin(), futs.end()).then_wrapped([](ss::future<std::vector<ss::future<>>> &&res) {
        if (res.failed()) {
